...
- `apps/server/src/index.ts` imports `@mono-hono-electron-react/api` and runs it on the Node server.
- `apps/desktop/src/App.tsx` imports `AppType` from `@mono-hono-electron-react/api` and calls the route through `hc`.
